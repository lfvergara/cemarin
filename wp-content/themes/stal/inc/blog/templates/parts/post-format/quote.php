...
if ( ! empty( $quote_text ) ) {
	$quote_position     = get_post_meta( get_the_ID(), 'qodef_post_format_quote_position', true );
	...
	$author_title_tag = isset( $author_title_tag ) && ! empty( $author_title_tag ) ? $author_title_tag : 'h4';
	?>
	<div class="qodef-e-quote">
	    <?php stal_render_icon( 'icon-basic-message', 'linea-icons', '' ); ?>
		<<?php echo esc_attr( $title_tag ); ?> class="qodef-e-quote-text"><?php echo esc_html( $quote_text ); ?></<?php echo esc_attr( $title_tag ); ?>>
        <?php if ( ! empty( $quote_position ) ) { ?>
            <span class="qodef-e-quote-author-position"><?php echo esc_html( $quote_position ); ?></span>
        <?php } ?>
