...
if ( ! class_exists( 'StalWelcomePage' ) ) {
	class StalWelcomePage {
		/**
		 * singleton class
		 */
		...
		/**
		 * get the instance of StalWelcomePage
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_stal_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_stal_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_stal_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'stal_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'stal_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( STAL_ROOT_DIR . '/screenshot.png' ) ? STAL_ROOT . '/screenshot.png' : STAL_ROOT . '/screenshot.jpg';

			stal_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			if ( $hook === 'appearance_page_stal_welcome_page' ) {
				wp_enqueue_style( 'stal-welcome-page-style', STAL_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}
}

StalWelcomePage::get_instance();