...
if ( ! function_exists( 'stal_add_rest_api_pagination_global_variables' ) ) {
	function stal_add_rest_api_pagination_global_variables( $global, $namespace ) {
	...
	add_filter( 'stal_filter_rest_api_global_variables', 'stal_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'stal_add_rest_api_pagination_route' ) ) {
	function stal_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'stal_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'stal' )
				)
			)
		);
	...
	add_filter( 'stal_filter_rest_api_routes', 'stal_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'stal_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function stal_get_new_posts() {
		
		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			stal_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'stal' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = stal_get_query_params( $options );
				...
				echo apply_filters( "stal_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) );
				
				$html = ob_get_contents();
				
				ob_end_clean();
				
				stal_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'stal' ), $html );
			} else {
				stal_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'stal' ) );
			}
		}
	}
}

if ( ! function_exists( 'stal_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_get_query_params( $params ) {
		...
		return apply_filters( 'stal_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'stal_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = stal_core_get_space_value( $params['space'] );
			}
...
if ( ! function_exists( 'stal_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function stal_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'stal' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%'
	...
	add_action( 'stal_action_after_blog_single_content', 'stal_add_link_pages_after_content' );
	add_action( 'stal_action_after_page_content', 'stal_add_link_pages_after_content' );
}
