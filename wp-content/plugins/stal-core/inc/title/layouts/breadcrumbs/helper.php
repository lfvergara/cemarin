...
if ( ! function_exists( 'stal_core_register_breadcrumbs_title_layout' ) ) {
	function stal_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'StalCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'stal_core_filter_register_title_layouts', 'stal_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'stal_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'stal-core' );
		
		return $layouts;
	}
	
	add_filter( 'stal_core_filter_title_layout_options', 'stal_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'stal_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function stal_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'stal_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'stal-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'stal-core' ),
			'author'      => esc_html__( 'Posted by %s', 'stal-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'stal-core' ),
			'404'         => esc_html__( '404 - Page not found', 'stal-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'stal-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'stal_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'stal_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'stal_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}