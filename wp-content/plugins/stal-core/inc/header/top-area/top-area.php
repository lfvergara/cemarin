...
class StalCoreTopArea {
	private static $instance;
	private $is_top_area_enabled;
	private $is_top_area_in_grid;
	...
	public function __construct() {
		...
		add_action( 'stal_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'stal_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
	}
	...
	function is_top_area_enabled() {
		$top_area_enabled = stal_core_get_post_value_through_levels( 'qodef_top_area_header' ) === 'yes';
	...
	function is_top_area_in_grid() {
		$top_area_in_grid = stal_core_get_post_value_through_levels( 'qodef_top_area_in_grid' ) === 'yes';
		
		if ( is_404() ) {
			$top_area_in_grid = false;
		}
		
		$this->is_top_area_in_grid = $top_area_in_grid;
	}
	
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = stal_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
			$top_bar_height = ! empty( $height ) ? $height : 45;
	...
	public function is_top_area_transparent() {
		$background_color = stal_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		$parameters = array(
			...
			'top_area_in_grid' => $this->is_top_area_in_grid,
		);
		
		stal_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$background_color = stal_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
		$height           = stal_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$padding          = stal_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
