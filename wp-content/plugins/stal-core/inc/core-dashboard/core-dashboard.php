...
if ( ! class_exists( 'StalCoreDashboard' ) ) {
	class StalCoreDashboard {
		private static $instance;

		private $sub_pages = array();
		private $validation_url = 'https://api.qodeinteractive.com/purchase-code-validation.php';
		public $licence_field = 'stal_purchase_info';
		public $import_field = 'stal_import_params';
		...
		function __construct() {
			...
			add_action( 'stal_core_action_on_deactivation', array( &$this, 'remove_redirect' ) );

			if ( get_option( 'stal_core_activated_first_time' ) === 'yes' ) {
				add_action( 'stal_core_action_plugin_loaded', array( &$this, 'page_welcome_redirect' ) );
			}
		...
		public function set_sub_pages( StalCoreSubPage $sub_page ) {
		...
		function load_files() {
			include_once STAL_CORE_INC_PATH . '/core-dashboard/rest/include.php';
			include_once STAL_CORE_INC_PATH . '/core-dashboard/registration-rest.php';
			include_once STAL_CORE_INC_PATH . '/core-dashboard/core-dashboard-theme-validation.php';
			include_once STAL_CORE_INC_PATH . '/core-dashboard/sub-pages/sub-page.php';
			
			foreach ( glob( STAL_CORE_INC_PATH . '/core-dashboard/sub-pages/*/load.php' ) as $subpages ) {
		...
		function dashboard_add_page() {
			...
			$page = add_menu_page(
				esc_html__( 'Stal Dashboard', 'stal-core' ),
				esc_html__( 'Stal Dashboard', 'stal-core' ),
				'administrator',
				'stal_core_dashboard',
				array( &$this, 'load_dashboard_template' ),
				STAL_CORE_INC_URL_PATH . '/core-dashboard/assets/img/admin-logo-icon.png',
				998
			);
		...
		function load_dashboard_template() {
			$params                 = array();
			$params['theme_name']   = qode_framework_is_installed( 'theme' ) ? esc_html( wp_get_theme()->get( 'Name' ) ) : esc_html__( 'Qode Interactive', 'stal-core' );
			$params['system_info']  = StalCoreSystemInfoPage::get_instance()->get_system_info();
			$params['info']         = $this->purchased_code_info();
			$params['is_activated'] = ! empty( $this->get_purchased_code() );

			stal_core_template_part( 'core-dashboard', 'templates/core-dashboard', '', $params );
		}
		
		public function register_sub_pages() {
			$sub_pages = apply_filters( 'stal_core_filter_add_welcome_sub_page', $icons = array() );
		...
		function enqueue_styles() {
			...
			wp_enqueue_style( 'stal-core-dashboard-style', STAL_CORE_INC_URL_PATH . '/core-dashboard/assets/css/core-dashboard.min.css' );
		}
		...
		function enqueue_scripts() {
			...
			wp_enqueue_script( 'stal-core-dashboard-script', STAL_CORE_INC_URL_PATH . '/core-dashboard/assets/js/modules/core-dashboard.js', array(), false, true );
			$global_variables = apply_filters( 'stal_core_filter_dashboard_js_global_variables', array() );

			wp_localize_script( 'stal-core-dashboard-script', 'qodefCoreDashboardGlobalVars', array(
				'vars' => $global_variables,
			) );
		}

		function remove_redirect() {
			delete_transient( 'stal_core_welcome_page_redirect' );
		}

		function page_welcome_redirect() {
			$redirect = get_transient( 'stal_core_welcome_page_redirect' );
			
			if ( empty( $redirect ) ) {
				set_transient( 'stal_core_welcome_page_redirect', 1, 31536000 );
				
				wp_safe_redirect( add_query_arg( array( 'page' => 'stal_core_dashboard' ), esc_url( admin_url( 'admin.php' ) ) ) );
			}
		}
		...
		function purchase_code_registration() {
			...
			if ( ! isset( $_POST ) || empty( $_POST ) ) {
				return esc_html__( 'All fields are empty', 'stal-core' );
			} else {
		...
		function register_purchase_code() {
			...
			if ( empty( $data['purchase_code'] ) || empty( $data['email'] ) ) {
				qode_framework_get_ajax_status( 'error', esc_html__( 'Purchase Code and Email are empty', 'stal-core' ), array(
					'purchase_code' => false,
					'email'         => false
				) );
			} elseif ( empty( $data['purchase_code'] ) ) {
				qode_framework_get_ajax_status( 'error', esc_html__( 'Purchase Code is empty', 'stal-core' ), array( 'purchase_code' => false ) );
			} elseif ( empty( $data['email'] ) ) {
				qode_framework_get_ajax_status( 'error', esc_html__( 'Email is empty', 'stal-core' ), array( 'email' => false ) );
			}

			$url = add_query_arg( array(
				'purchase_code' => rtrim( $data['purchase_code'] ),
				'email'         => $data['email'],
				'profile'       => STAL_CORE_PROFILE_SLUG . '-themes',
		...
		function api_connection( $url ) {
			...
			if ( 200 !== intval( $response_code ) ) {
				return new WP_Error( 'bad_request', esc_html__( 'Bad request', 'stal-core' ) );
			}

			$json = json_decode( wp_remote_retrieve_body( $response ), true );

			if ( empty( $json ) || ! is_array( $json ) ) {
				return new WP_Error( 'invalid_response', esc_html__( 'Invalid Response', 'stal-core' ) );
			}
		...
		function response_codes( $code, $data = array() ) {
			...
			switch ( $code ):
				...
				case 200:
					$message = esc_html__( 'Failed to validate code due to an error', 'stal-core' );
					break;
				case 400:
					$message = esc_html__( 'Parameter or argument in the request was invalid', 'stal-core' );
					break;
				case 401:
					$message = esc_html__( 'The authorization header is missing. Verify that your code is correct.', 'stal-core' );
					break;
				case 403:
					$message = esc_html__( 'Personal token is incorrect or does not have the required permission(s)', 'stal-core' );
					break;
				case 404:
					$message = esc_html__( 'The purchase code is invalid', 'stal-core' );
					break;
				case 601:
					$message = esc_html__( 'You successfully activated theme', 'stal-core' );
					break;
				case 602:
					$message = esc_html__( 'Code is valid', 'stal-core' );
					break;
				case 603:
					$message = esc_html__( 'You successfully added demo', 'stal-core' );
					break;
				case 604:
					$message = esc_html__( 'You successfully deregister theme', 'stal-core' );
					break;
				case 650:
					$registered_url = '';
					...
					break;
				case 651:
					$message = esc_html__( 'Error occurred during activation', 'stal-core' );
					break;
				case 652:
					$message = esc_html__( 'Code is invalid', 'stal-core' );
					break;
				case 653:
					$message = esc_html__( 'Error occurred during adding', 'stal-core' );
					break;
				case 654:
					$message = esc_html__( 'Error occurred during deactivation', 'stal-core' );
					break;
			endswitch;
	...
	StalCoreDashboard::get_instance();
}