...
abstract class StalCoreMobileHeader {
	...
	public function __construct() {
		...
		add_filter( 'stal_filter_add_inline_style', array( $this, 'set_inline_mobile_header_styles' ) );
		add_filter( 'stal_core_filter_content_margin_mobile', array( $this, 'get_content_margin' ) );
		add_filter( 'stal_core_filter_title_padding_mobile', array( $this, 'get_title_padding' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'stal_core_filter_mobile_header_template', $parameters );
		
		return stal_core_get_template_part( 'mobile-header/layouts/' . $this->slug, 'templates/' . $this->slug, '', $parameters );
	}
	
	public function set_inline_mobile_header_styles( $style ) {
		$item_styles = array();
		
		$height           = stal_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_height' );
		$background_color = stal_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_background_color' );
		$padding          = stal_core_get_post_value_through_levels( 'qodef_mobile_header_side_padding' );
		...
		if ( ! empty( $padding ) ) {
			$item_styles['padding'] = '0 ' . $padding;
			$style .= qode_framework_dynamic_style( '.qodef-mobile-header--' . $this->slug . ' #qodef-page-mobile-header #qodef-page-mobile-header-inner', array( 'padding' => $item_styles['padding'] ) );
		}
	...
	function set_header_height() {
		$header_height = stal_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'stal_core_filter_set_mobile_header_height', $header_height );
	}
