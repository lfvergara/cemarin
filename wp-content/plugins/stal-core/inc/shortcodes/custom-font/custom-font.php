...
if ( ! function_exists( 'stal_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'StalCoreCustomFontShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'stal_core_filter_register_shortcodes', 'stal_core_add_custom_font_shortcode' );
}

if ( class_exists( 'StalCoreShortcode' ) ) {
	class StalCoreCustomFontShortcode extends StalCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'stal_core_filter_custom_font_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( STAL_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'stal_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'stal-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'stal-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'title',
				'title'      => esc_html__( 'Title Text', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'title_tag',
			...
			$this->set_option( array(
				...
				'name'        => 'font_size_1366',
				'title'       => esc_html__( 'Font Size', 'stal-core' ),
				'description' => esc_html__( 'Set responsive style value for screen size 1366', 'stal-core' ),
				'group'       => esc_html__( 'Screen Size 1366', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type'  => 'text',
			...
			$this->set_option( array(
				...
				'name'        => 'font_size_1024',
				'title'       => esc_html__( 'Font Size', 'stal-core' ),
				'description' => esc_html__( 'Set responsive style value for screen size 1024', 'stal-core' ),
				'group'       => esc_html__( 'Screen Size 1024', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type'  => 'text',
			...
			$this->set_option( array(
				...
				'name'        => 'font_size_768',
				'title'       => esc_html__( 'Font Size', 'stal-core' ),
				'description' => esc_html__( 'Set responsive style value for screen size 768', 'stal-core' ),
				'group'       => esc_html__( 'Screen Size 768', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type'  => 'text',
			...
			$this->set_option( array(
				...
				'name'        => 'font_size_680',
				'title'       => esc_html__( 'Font Size', 'stal-core' ),
				'description' => esc_html__( 'Set responsive style value for screen size 680', 'stal-core' ),
				'group'       => esc_html__( 'Screen Size 680', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type'  => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return stal_core_get_template_part( 'shortcodes/custom-font', 'variations/'.$atts['layout'].'/templates/custom-font', '', $atts );
		}
		...
		private function set_responsive_styles( $atts ) {
			...
			foreach ( $screen_sizes as $screen_size ) {
				...
				if ( ! empty( $styles ) ) {
					add_filter( 'stal_core_filter_add_responsive_' . $screen_size . '_inline_style_in_footer', function ( $style ) use ( $unique_class, $styles ) {
