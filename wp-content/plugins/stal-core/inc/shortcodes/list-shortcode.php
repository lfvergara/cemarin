...
abstract class StalCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
    ...
    public function register_list_scripts() {
        ...
        $list_scripts = apply_filters( 'stal_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
	...
	public function map_list_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'behavior',
			'title'         => esc_html__( 'List Appearance', 'stal-core' ),
			'options'       => stal_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Image Proportions', 'stal-core' ),
				'options'       => array(
					''      => esc_html__( 'Original', 'stal-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'stal-core' ),
				'options'       => stal_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'stal-core' ),
				'description' => esc_html__( 'Enter image width in px', 'stal-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'stal-core' ),
				'description' => esc_html__( 'Enter image height in px', 'stal-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => stal_core_get_select_type_options_pool(
						'columns_number',
						true,
						$exclude_columns,
						$include_columns
			...
			$this->set_option(
				array(
					...
					'options'       => stal_core_get_select_type_options_pool( 'columns_responsive' ),
			...
			$this->set_option(
				array(
					...
					'name'          => 'columns_1440',
					'title'         => esc_html__(
						'Number of Columns 1367px - 1440px',
						'stal-core'
					),
			...
			$this->set_option(
				array(
					...
					'name'          => 'columns_1366',
					'title'         => esc_html__(
						'Number of Columns 1025px - 1366px',
						'stal-core'
					),
					'options'       => stal_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
			...
			$this->set_option(
				array(
					...
					'name'          => 'columns_1024',
					'title'         => esc_html__(
						'Number of Columns 769px - 1024px',
						'stal-core'
					),
					'options'       => stal_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'stal-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'stal-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'stal-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_loop',
			'title'      => esc_html__( 'Enable Slider Loop', 'stal-core' ),
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Autoplay', 'stal-core' ),
			'options'    => stal_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'stal-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'stal-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'stal-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'stal-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Choose Slide Animation', 'stal-core' ),
			'options'    => array(
				''     => esc_html__( 'Default (Slide)', 'stal-core' ),
				'fade' => esc_html__( 'Fade', 'stal-core' ),
			),
			'dependency' => $dependency,
			'group'      => $group,
			)
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Navigation', 'stal-core' ),
			'options'    => stal_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Pagination', 'stal-core' ),
			'options'    => stal_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = stal_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}

		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ) ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => stal_core_get_custom_image_size_class_name( $atts['images_proportion'] )
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? stal_core_get_space_value( $atts['space'] ) : 0;
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'stal-core' );
		...
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'posts_per_page',
			'title'      => esc_html__( 'Posts per Page', 'stal-core' ),
			'default'    => '-1',
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'stal-core' ),
			'options'       => stal_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'stal-core' ),
			'options'       => stal_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Additional Params', 'stal-core' ),
			'options'    => array(
				''    => esc_html__( 'No', 'stal-core' ),
				'id'  => esc_html__( 'Post IDs', 'stal-core' ),
				'tax' => esc_html__( 'Taxonomy Slug', 'stal-core' ),
			),
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'post_ids',
			'title'       => esc_html__( 'Posts IDs', 'stal-core' ),
			'description' => esc_html__( 'Separate post IDs with commas', 'stal-core' ),
			'group'       => $group,
			'dependency'  => array(
				'show' => array(
		...
		if ( ! empty( $taxonomies_formatted ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'tax',
				'title'      => esc_html__( 'Taxonomy', 'stal-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'tax_slug',
				'title'      => esc_html__( 'Taxonomy Slug', 'stal-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'tax__in',
				'title'       => esc_html__( 'Taxonomy IDs', 'stal-core' ),
				'description' => esc_html__( 'Separate taxonomy IDs with commas', 'stal-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'additional_params' => array(
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'layout',
			'title'         => esc_html__( 'Item Layout', 'stal-core' ),
			'options'       => $layouts,
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'stal-core' ),
	...
	public function map_additional_options( $params = array() ) {
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				                   'field_type'    => 'select',
				                   'name'          => 'enable_filter',
				                   'title'         => esc_html__( 'Enable Filter', 'stal-core' ),
				                   'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'stal-core' ),
				                   ...
				                   'group'         => esc_html__( 'Additional', 'stal-core' )
			                   ) );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Pagination', 'stal-core' ),
				'options'       => stal_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'stal-core' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'stal-core' ),
				'dependency'    => array(
					'show' => array(
	...
	public function load_assets() {
		do_action( 'stal_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
