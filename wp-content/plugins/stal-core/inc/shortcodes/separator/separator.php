...
if ( ! function_exists( 'stal_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'StalCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'stal_core_filter_register_shortcodes', 'stal_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'StalCoreShortcode' ) ) {
	class StalCoreSeparatorShortcode extends StalCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( STAL_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'stal_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'stal-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'stal-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'stal-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'stal-core' ),
				'options'    => stal_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'stal-core' ),
				'group'      => esc_html__( 'Style', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'stal-core' ),
				'group'      => esc_html__( 'Style', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'stal-core' ),
				'group'      => esc_html__( 'Style', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'stal-core' ),
				'group'      => esc_html__( 'Style', 'stal-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return stal_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
