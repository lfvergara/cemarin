...
if ( ! function_exists( 'stal_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function stal_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'StalCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'stal_core_filter_register_shortcodes', 'stal_core_add_banner_shortcode' );
}

if ( class_exists( 'StalCoreShortcode' ) ) {
	class StalCoreBannerShortcode extends StalCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'stal_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'stal_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( STAL_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'stal_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'stal-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'stal-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'subtitle_tag',
				'title'         => esc_html__( 'Subtitle Tag', 'stal-core' ),
				'options'       => stal_core_get_select_type_options_pool( 'title_tag' ),
				'default_value' => 'h5',
				'group'         => esc_html__( 'Content', 'stal-core' )
			) );
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Subtitle Margin Top', 'stal-core' ),
				'group'      => esc_html__( 'Content', 'stal-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return stal_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
